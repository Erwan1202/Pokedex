<template>
  <div ref="root" class="relative">
    <button
      @click="toggle"
      type="button"
      class="h-10 px-4 rounded-md bg-white text-gray-700 text-sm font-medium
    ...
    <div v-show="open"
      class="absolute z-50 mt-2 w-48 rounded-lg bg-white p-2
             ring-1 ring-gray-200 shadow-[0_10px_24px_rgba(0,0,0,0.12)]">
      <div class="max-h-64 overflow-auto">
        <label v-for="opt in items" :key="opt.id"
          class="flex items-center gap-2 px-2 py-2 rounded-md hover:bg-gray-50 cursor-pointer select-none">
          <input
